...
def calculate_statistics(csv_file):
    ...
    # Calculate mean, median, and standard deviation for each exam
    exam_stats = {}
    for exam in df.columns[2:-1]:  # Exclude "Roll_Number", "Name", and "total" columns
        ...
        std_dev = round(exam_marks.std(skipna=True), 2)  # Round standard deviation to 2 decimal places
        exam_stats[exam] = {"Mean": mean, "Median": median, "Standard Deviation": std_dev}
...
if __name__ == "__main__":
    ...
    # Print statistics results including standard deviation
    print("Statistics for each exam:")
    for exam, values in stats.items():
        print(f"{exam}: Mean={values['Mean']}, Median={values['Median']}, Std Dev={values['Standard Deviation']}")
